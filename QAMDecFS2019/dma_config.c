...
void vInitDMA()
{
	...
	DMA.CH0.TRFCNT		= DMA_BUF_LEN; // always the number of bytes, even if burst length > 1
	...
	DMA.CH1.ADDRCTRL	= DMA_CH_SRCRELOAD_BURST_gc | // reload source after every burst
	DMA_CH_DESTRELOAD_TRANSACTION_gc | DMA_CH_DESTDIR_INC_gc; // reload destination after every transaction
	DMA.CH1.TRIGSRC		= DMA_CH_TRIGSRC_ADCB_CH0_gc; //DMA1 gets synched by ADCB CH0
	DMA.CH1.TRFCNT		= DMA_BUF_LEN; // always the number of bytes, even if burst length > 1
